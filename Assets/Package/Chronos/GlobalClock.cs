...
namespace Chronos
{
	...
	public class GlobalClock : Clock
	{
		...
		internal override void OnChangedTimeScale(float newTimeScale)
		{
			base.OnChangedTimeScale(newTimeScale);

			foreach (Clock clock in clocks.Where(w => w != null))
			{
				clock.OnChangedTimeScale(newTimeScale);
			}

			foreach (Timeline timeline in timelines.Where(w => w != null))
			{
				timeline.ObserveTimeScale(newTimeScale);
			}
		}
	}
}
